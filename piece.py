...
class Pawn(Piece):
  ...
  def returnValidFields(self, board):
    ...
    p = [self.x_position, self.y_position+1*upDown]
    if board.isFieldValid(p[0], p[1]) and board.isFieldEmpty(p[0], p[1]):
      validFields.append(p)
    ...
    if self.initialMoveNotDone:
      ...
      if board.isFieldValid(p[0], p[1]) and board.isFieldEmpty(p[0], p[1]) and board.isFieldEmpty(p[0], p[1]):
        validFields.append(p)
